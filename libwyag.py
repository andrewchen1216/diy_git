...
def main(argv=sys.argv[1:]):
    ...
    if args.command == "add":
        ...
        cmd_commit(args)
    elif args.command == "hash-cd ..object":
...
class GitRepo(object):
    ...
    def __init__(self, path, force=False):
        ...
        if not force:
            ...
            if vers != 0:
                raise Exception("Unsupported repo format version %s" % vers)
...
def object_read(repo, sha):
    ...
    with open(path, "rb") as f:
        ...
        x = raw.find(b' ')
        fmt = raw[0:x]
        # Read and Validate Object
        y = raw.find(b'\x00', x)
        size = int(raw[x:y].decode("ascii"))
        if size != len(raw) - y - 1:
            raise Exception("Malformed Object{0}:bad length".format(
...
argsp.add_argument("path", help="Read object form <file>")
